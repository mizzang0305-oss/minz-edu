...
export function GaugePanel({ battle }: { battle: CoopBattleState }) {
  return (
    <aside className="gauge-panel" aria-label="전투 상태">
      ...
      <div className="boss-meter"><div className="meter-label"><span>{battle.players.length === 2 ? "쌍둥이 숫자 슬라임" : "숫자 슬라임"}</span><strong>{battle.bossHp}/{battle.bossMaxHp}</strong></div><div className="meter-track boss" role="progressbar" aria-label="보스 생명력" aria-valuemin={0} aria-valuemax={battle.bossMaxHp} aria-valuenow={battle.bossHp}><span style={{ width: `${(battle.bossHp / battle.bossMaxHp) * 100}%` }} /></div>{battle.bossShield > 0 && <span className="shield-chip">보호막 {battle.bossShield}</span>}</div>
    </aside>
  );
}
