...
async function enterLandscapeGameplay(page: import("@playwright/test").Page) {
  const viewport = page.viewportSize();
  if (!viewport) return;
  if (viewport.width <= 900 && viewport.height > viewport.width) {
    await page.setViewportSize({ width: viewport.height, height: viewport.width });
  }
  await expect(page.getByRole("dialog", { name: "가로 화면 안내" })).toBeHidden();
}

async function finishExploration(page: import("@playwright/test").Page) {
  await enterLandscapeGameplay(page);
...
async function saveSetupAndOpenWorld(page: import("@playwright/test").Page) {
  ...
  await expect(page).toHaveURL(/\/world$/, { timeout: 15_000 });
}
...
test("같은 기기 2인 협동 전투와 새로고침 저장", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: /번개 베기/ }).click();
  ...
  await expect(page.getByRole("heading", { name: "민표의 장비·스킬 상점" })).toBeVisible();
  await expect(page.locator(".functional-inventory-grid article")).toHaveCount(2);
  await expect(page.getByText("번개 베기", { exact: true }).first()).toBeVisible();
  ...
  await page.getByRole("link", { name: "STATUS로 돌아가기" }).click();
...
test("주별 목표를 진단하고 이미 아는 목표를 건너뛸 수 있다", async ({ page }) => {
  ...
  expect(await page.locator(".week-goal-card").count()).toBe(20);
  await page.getByRole("button", { name: "이미 아는지 확인" }).first().click();
  await expect(page).toHaveURL(/\/training\?mode=diagnostic/);
  await page.getByRole("button", { name: "50", exact: true }).click();
  await page.getByRole("button", { name: "해설 확인하고 다음 문제" }).click();
  await page.getByRole("button", { name: "470", exact: true }).click();
  await page.getByRole("button", { name: "해설 확인하고 다음 문제" }).click();
  await page.getByRole("button", { name: "632", exact: true }).click();
  await page.getByRole("button", { name: "해설 확인하고 훈련 완료" }).click();
  await expect(page.getByRole("heading", { name: /게임 내 준비됨/ })).toBeVisible();
  await page.getByRole("link", { name: "다음 주차 고르기" }).click();
  await expect(page.locator(".week-goal-card").first()).toContainText("완료 · 1회 시도 · 3문제 기록");
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
});
