...
export function validateMapDefinition(map: ExplorationMapDefinition) {
  const errors: string[] = [];
  const ids = [map.bridge.id, map.secretArea.id, map.battleSafeArea.id, map.boss.id, map.chest.id, ...map.collectibles.map((item) => item.id), ...map.obstacles.map((item) => item.id), ...map.npcs.map((item) => item.id)];
  ...
  if (!rectInBounds(map, map.bridge) || !rectInBounds(map, map.secretArea) || !rectInBounds(map, map.battleSafeArea) || map.obstacles.some((obstacle) => !rectInBounds(map, obstacle))) errors.push("all rectangles must have positive size and stay in bounds");
