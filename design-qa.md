...
- Viewports checked: 390x844 portrait phone, 844x390 landscape phone, 478x850 reference size, 768x1024 tablet, 1366x768 desktop.
...
- Responsive behavior: a portrait phone shows a dedicated rotate-to-landscape gate instead of stretching the map; short landscape screens keep the game on the left and move the question dock to the right.
...
4. P1: forcing the 29:16 map into a portrait viewport distorted the world and removed useful movement space.
   - Fix: the canvas now preserves its 29:16 aspect ratio, the PWA requests landscape, and portrait phones show a clear full-screen rotation action.
   - Post-fix evidence: 390x844 shows only the rotate guide; 844x390 removes the gate and renders the complete combat HUD without document scrolling.
5. P1: player, boss, and name labels could remain behind the right-side question HUD after a phone rotation.
   - Fix: every Tiled map now declares one `battle_safe_area`; the scene recalculates a compact formation whenever the viewport changes and scales only the phone-layout actors.
   - Post-fix evidence: 844x390 keeps the boss, phase label, both players, and their names to the left of the command dock.
