...
키는 `minz-learning-game`, 현재 스키마 버전은 `2`이다. 저장 항목은 프로필, 보호자 설정, 접근성, 개념 진행, 인벤토리, 보상/생각/플레이 이력, 친구 임시 프로필, 로컬 협동 설정, 협동 이력, 팀 보상, 해금된 팀 스킬, 협동 UX 관찰 기록이다. v1 데이터는 필드를 보존한 채 v2 기본값을 채우며, 파싱 실패나 알 수 없는 버전은 안전한 기본값으로 복구한다.
