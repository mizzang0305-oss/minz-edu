...
- PR Preview에서 부모 테스트 계정으로 학습 세션 1회를 완료하고 보호자 요약 메일 정확히 1건을 발송했다. 앱 상태 `보호자 메일 발송 완료`와 Resend `delivered` 이벤트를 확인했다.
- 발송 본문은 세션 합계·학습 목표·복습 관찰만 포함하며 문제 원문, 선택 답, 계정 UID, 자녀 문서 ID를 포함하지 않았다.
- 검증용 Firebase 승인 도메인과 사용하지 않은 임시 Resend API 키는 검증 직후 제거했다.
...
1. 실제 휴대폰 2대에서 온라인 팀전 재접속과 턴 충돌을 검증한다.
2. App Check를 모니터링 모드부터 적용하고 실제 기기 요청을 확인한다.
