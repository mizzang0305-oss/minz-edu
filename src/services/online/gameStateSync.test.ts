...
describe("guardian game state sync boundary", () => {
  ...
  it("deduplicates the same invalid legacy record regardless of array order", () => {
    const sharedLeft = adventure("../shared-private", 10, "invalid");
    const sharedRight = adventure("../shared-private", 10, "invalid");
    const filler = adventure("valid-filler", 0, "2026-07-15T06:10:00.000Z");
    const left = createGameSyncSnapshot(gameWith([filler, sharedLeft]));
    const right = createGameSyncSnapshot(gameWith([sharedRight]));
    const leftShared = left.adventures.find((item) => item.id.startsWith("legacy-adventure-"));
    const rightShared = right.adventures.find((item) => item.id.startsWith("legacy-adventure-"));
    const merged = mergeGameSyncSnapshots(left, right);

    expect(leftShared?.id).toBe(rightShared?.id);
    expect(merged.adventures.filter((item) => item.id.startsWith("legacy-adventure-"))).toHaveLength(1);
    expect(merged.adventures).toHaveLength(2);
  });

  it("deduplicates IDs generated by the briefly deployed index-based format", () => {
    const record = adventure("../shared-private", 35, "invalid");
    const current = createGameSyncSnapshot(gameWith([record]));
    const currentId = current.adventures[0].id;
    const previousId = currentId.replace(/^legacy-adventure-/, "legacy-adventure-0-");
    const previous = {
      ...current,
