...
export function InventoryClient() {
  ...
  return (
    <main className="inventory-page game-inventory-page">
      ...
      <section className="functional-inventory-grid">{owned.map((item) => <article key={item.id} className={isEquipped(item.id) ? "equipped" : ""}><span>{item.type === "weapon" ? "무기" : item.type === "armor" ? "방어구" : "스킬"}</span><h3>{item.name}</h3><p>{item.description}</p><button type="button" disabled={isEquipped(item.id)} onClick={() => use(item.id)}>{isEquipped(item.id) ? "사용 중" : item.type === "skill" ? "전투 스킬로 선택" : "장착하기"}</button></article>)}</section>
